...
/** Type-safe CSS variable style for --dx/--dy (no 'any'). */
type SlashVarStyle = CSSProperties & {
    ['--dx']?: string;
    ['--dy']?: string;
};
    ...
    return (
        ...
        >
            ...
            {damageOn && !paused && (
                ...
                >
                    {slashes.map((s, i) => {
                        // typed CSS variables for --dx/--dy
                        const coreVars: SlashVarStyle = { ['--dx']: `${s.dx}px`, ['--dy']: `${s.dy}px` };
                        const hazeVars: SlashVarStyle = {
                            ['--dx']: `${Number(s.dx) * 0.6}px`,
                            ['--dy']: `${Number(s.dy) * 0.6}px`,
                        };
                                ...
                                {burstOn && !paused && (
                                    ...
                                    >
                                        ...
                                        {shardVectors.map((s, i) => {
                                            const styleVars: CSSProperties & Record<'--tx' | '--ty' | '--r', string> = {
                                                ['--tx']: `${s.tx}px`,
                                                ['--ty']: `${s.ty}px`,
                                                ['--r']: `${s.r}deg`,
                                            };
